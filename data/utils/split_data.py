...
parser.add_argument(
    "--frac", help="fraction in training set; default: 0.9;", type=float, default=0.9
)
parser.add_argument(
    "--seed",
...
if os.environ.get("LEAF_DATA_META_DIR") is not None:
    ...
    with open(seed_fname, "w+") as f:
        f.write(
            "# split_seed used by sampling script - supply as "
...
=======
            ...
            user_files.extend(
                [(u, ns, f) for (u, ns) in zip(data["users"], data["num_samples"])]
            )
>>>>>>> f451921 (Not reload same file multiple times)
        ...
        for i, u in enumerate(data["users"]):
            ...
            if curr_num_samples >= 2:
                ...
                if args.name in ["shakespeare"]:
                    ...
                    test_indices = [
                        i for i in range(num_train_samples + 80 - 1, curr_num_samples)
                    ]
                else:
